    ...
    <div id="azen-produc">
        <div class="azen-tittop clearfix">
            <div class="azen-titles fl clearfix">
                <h3>会员注册 <img src="themes/img/icon/sc.png" width="17" height="23" title="" alt=""></h3>
                <p class="azen-titlep clearfix"><span class="fl">会员注册页面完成注册后可分享作品，评论，点赞，收藏和意见反馈。</span><i class="fa fa-edit"></i> </p>
                    ...
                    <fieldset>
                        <form action="${pageContext.request.contextPath }/userregister.action" method="post" onsubmit="return check()">
                            ...
                            <p class="clearfix">
                                ...
                                <input id="rpassword" class="azen-username" type="password" name="password">
                            </p>
                            <p class="clearfix">
    ...
    $(document).ready(function(){
    	...
    	$("#rusername").blur(
    			function(){
    				$(".rname_lable").empty();
    				if(this.value==""){
    					$("#rusername").after("<label class='rname_lable'><font>用户名不能为空</font></label>");
    				}else if(this.value.length<6){
    					$("#rusername").after("<label class='rname_lable'><font>用户名不能小于6位</font></label>");
    				}
    			}		
    	)
    	$("#rpassword").blur(
    			function(){
    				...
    				if(this.value==""){
    					$("#rpassword").after("<label class='rpassword_lable'><font>密码不能为空</font></label>");
    				}else if(this.value.length<6){
    					$("#rpassword").after("<label class='rpassword_lable'><font>密码不能小于6位</font></label>");
    				}
    			}		
    	)
    	$("#repassword").blur(
    			...
    			function(){
    				...
    				if(this.value==""){
    					$("#repassword").after("<label class='repassword_lable'><font>密码不能为空</font></label>");
    				}else if(this.value.length<6){
