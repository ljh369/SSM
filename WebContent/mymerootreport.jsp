...
List<Report> report=(List)request.getAttribute("report");
            ...
            <div style="height: auto;">
        		<%if(report.size()!=0){ 
                	for(int i=0;i<report.size();i++){
                		Report report1 =new Report(); //新建对象
                		report1 = report.get(i);   //每循环一次后将此时的值保存到对象里%>
        		<div style="margin-top:50px;">
        				<a href="#" style="padding:30px;margin-left: 100px;margin-top: 50px" ><img alt="" src="<%=report1.getUserimgurl() %>" height="50px" width="50px"></a>
        				<div style="margin-left: 200px">
        					<div id="name" style="padding-bottom: 20px;color:#FF1493; font-size: 15px"><%= report1.getUser()%></div>
        					<div id="content" style="padding-bottom: 20px;font-size: 20px"><%= report1.getCommentcontent()%></div>
        					<div>举报者：<%= report1.getReportuser()%></div>
        					<a href="${pageContext.request.contextPath }/deletereport.action?reportid=<%=report1.getReportid() %>&id=1&commentid=<%=report1.getCommentid() %>" style="float: right;margin-right: 100px">删除评论</a>
        					<a href="${pageContext.request.contextPath }/deletereport.action?reportid=<%=report1.getReportid() %>&id=2" style="float: right;margin-right: 100px">评论无问题</a>
        					<div id="time" style="padding-bottom: 20px;color: gray;">评论日期：<%= report1.getCommentdate()%></div>
        					<hr />
        				</div>
        		</div>
        		<%	}
...
<footer id="azen-footer">
    <div class="azen-footer">
        ...
        <p><a href="http://azenui.com" style="color:#f60; text-decoration:underline;"></a> </p>
    </div>
</footer>
